const sentenceData = getIrregularWordsSimplePast();
...
function answerValidator(inputField, correctAnswer, correctAnswer2){
  if ((inputField.value.toLowerCase() === correctAnswer)||(inputField.value.toLowerCase() === correctAnswer2)) {
...
submitButton.addEventListener('keypress', (e) =>{
    if (e.keyCode === 13 && (answerValidator(inputPresentTense, answer) || answerValidator(inputPresentTense, answer2))) {
      correctAnswerAfter();
    }
  });
  
  submitButton.addEventListener('click', () => {
    if (answerValidator(inputPresentTense, answer) || answerValidator(inputPresentTense, answer2)) {
      correctAnswerAfter();
    }
  });
...
// MODAL explanation:
const showExplanationButton = document.getElementById("explanationButton");
const explanationModal = document.getElementById("explanationModal");
const closeExplanationModal = document.getElementById("close-explanation-modal");
const closeExplanationModalEnd = document.getElementById("close-explanation-modal-end");
closeExplanationModalEnd

showExplanationButton.addEventListener("click", () => {

  toggleModal(explanationModal);
...
closeExplanationModalEnd.addEventListener("click", () => {
